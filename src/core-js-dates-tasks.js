...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${formattedHour}:${pad(minute)}:${pad(
    second
  )} ${ampm}`;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const schedule = [];
  const day = new Date(startPeriod);

  while (day <= endPeriod) {
    for (let i = 0; i < countWorkDays && day <= endPeriod; i += 1) {
      schedule.push(formattingDate(day));
      day.setDate(day.getDate() + 1);
    }
    day.setDate(day.getDate() + countOffDays - 1);
  }
